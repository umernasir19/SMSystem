    ...
    public class SaleController : Controller
    {
        ...
        public JsonResult SearchSales(SalesReturn_Property objsale)
        {
            using (var db = new RAJPUT_RICE_DBEntities())
            {
                ...
                var purchasemaster =  db.sales.Where(p => p.idx == salesMasterId).FirstOrDefault();
...
where inv.stock>0 and sd.salesIdx=" + salesMasterId + "").ToList();
                ...
                //int saleqty = 0;
                //var invlogsdata =  db.inventory_logs.Where(P => P.MasterID == salesMaster.idx && P.TransactionTypeID == 17)
                //.GroupBy(o => o.MasterID)
                //   .Select(g => new { membername = g.Key, updatedquantity = g.Sum(i => i.stock) }).ToList();
                //// var invloggs =  db.inventory_logs.Where(p => p.TransactionTypeID == purchasemasterid).FirstOrDefault();
                //if (invlogsdata == null)
                //{
                //    saleqty = 0;
                //}
        ...
        public JsonResult CheckInverntoryforProductStock(int id)
        {
            using (var db = new RAJPUT_RICE_DBEntities())
            {
                ...
                {
                    //var data = (from a in  db.sales
                    //            join b in  db.salesDetails on a.idx equals b.salesIdx
                    //            join c in  db.accountMasterGLs on a.soNumber equals c.invoiceNoIdx
                    //            join d in  db.accountGJs on c.idxx equals d.GLIdx
                    //            join e in  db.inventories on b.serviceIdx equals e.productIdx
                    //            //join c in  db.sales on a.salesIdx equals c.idx
                    //            //join b in  db.inventories on a.serviceIdx equals b.productIdx
                    //            //join d in  db.accountMasterGLs on a. equals d.
